...
from agents.charms import CHARM_POLISH_VALIDATE
from agents.agents._text_agent import _TextAgent
...
class ValidatorAgent(_TextAgent):
    ...
    def __init__(self, model: Model):
        ...
        """
        # 内置验证工具
        format_validator = _SimplePassFailValidator()
    ...
    def run(self, text: str) -> str:
        ...
        for step in range(self.max_steps):
            ...
            # 使用内置的 _SimplePassFailValidator 收敛验证结果格式
            is_valid = self.tools[0](result)
        ...
        # loop 超限
        logger.debug(f"验证失败: {last_result}")
        return last_result
